...
ap=argparse.ArgumentParser(); ap.add_argument('--ckpt',required=True); ap.add_argument('--data',required=True); ap.add_argument('--allow-tokenizer-mismatch',action='store_true'); a=ap.parse_args(); device=pick_device(); model,ck=load_checkpoint(a.ckpt,SystemOneModel,device,allow_tokenizer_mismatch=a.allow_tokenizer_mismatch); model.to(device).eval(); tok=LabTokenizer(ck['tokenizer'])
