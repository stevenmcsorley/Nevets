...
**P2-PREP (infrastructure only; no runs before P0/P1 conclude)** · `src/systemone_lab/causal_worlds.py`: binary SCMs over named variables with exact CPTs. Structures: chain, mediator, independent (inherently see = do), and fork, confounder, collider, reverse (back-door, where seeing and doing can disagree). Queries P(T=1 | observe(V)) and P(T=1 | do(V)) are computed by exact enumeration, with the do-query on the mutilated graph. Pairs share the state and differ only in the condition phrase; `pair_kind` is differ or same, and back-door structures are rejection-sampled to ~60% differ. Wording families: 6 train, 4 held out (`observe_among`, `observe_learn`, `do_trial`, `do_switch`). Structures: 5 train, 2 held out (collider, reverse). Labels are kept ≥ 0.06 from the 0.5 threshold. Tests: do-queries match the mutilated graph (fork, chain, collider identities), pair invariants hold, wording families are disjoint, differ-rate is in range.

**P0-CHECK-1 (matched spatial exposure, verified from the actual sample)** · From GENERAL-1's diagnostics log (record IDs of 3,000 logged microbatches = 48,016 sampled records, all resolved to their domain through the mix), the empirical spatial share is **21.6%** (chains 16.0 + one-hop 5.6), matching the 22.0% implied by the sampling weights. M30 therefore saw ≈ **103,700 spatial examples** over 30,000 updates (~3.5 per update of 16); S6 sees **96,000** (7.4% fewer) over 6,000 updates (16 per update). **The S6 against M30 comparison is matched on spatial examples, not on updates**; S30 against M30 is matched on updates.

**TM-1-FLAG (confound, raised 26 September)** · TM-1 was matched within the experiment (data, updates, seed), but its inits were not matched for prior training: the SPATIAL init (exp6a) had **28,400** prior decision-training updates (P-04 800 → EB-1 1,200 → EB-3B 1,200 → X-1 1,200 → X-4 3,000 → X-5 15,000 → X-6A 6,000), BASE and scratch 0. "Spatial training helps other domains" is therefore **not controlled**: it is confounded with decision-format familiarity (question/option/decide layout, pointer scoring, entity binding) and total training. **Required control (future):** an init with ≈ 28k decision updates on non-spatial data (e.g. worlds without spatial), compared with exp6a on the same TM-1 protocol. Until then, the TM-1 claim stands only as "a decision-trained init transfers better than an LM or scratch init".

**PT-ADJ (owner adjustments, 26 September; recorded before any PT-2 run)**
1. **P3 held-out formats fixed now: pipe `table` and compact `symbolic` notation.** Table keeps continuity with GENERAL-1's documented weak spot (54%); symbolic is the most structurally foreign. `scripts/pt/build_corpus.py` defines `P3_HELDOUT_FORMATS`, and `structured_samples(..., for_pretraining=True)` excludes them (tested). The current pretraining shards contain FineWeb-Edu text only, with no synthetic structured slice. The 32k *tokenizer* training text does include all current formats, which is permitted.
2. **Reverse decontamination for eval sets built after the corpus** (PT-5; anything BoolQ/Wikipedia-derived): `scripts/pt/reverse_decontam.py` puts question-level anchors (normalized question text) and 13-gram passage anchors in one Aho-Corasick automaton and scans the corpus once, reporting hits per item (tested: a copied passage and question are flagged, an unrelated item is not). Contaminated items are dropped from the eval set before it is locked.
3. **PT-2 decision criterion (pre-registered):** 35M against 150M is judged on the **slope of the downstream probe across the 250M-token checkpoints**, not on the 1B-token endpoint; 150M is undertrained at 1B and is not rejected on the endpoint alone. The per-token downstream gain (Δ probe / Δ tokens), and whether the curve is still rising at 1B, decide the PT-3 size.
4. **Attribution:** FineWeb-Edu ODC-By v1.0 attribution is added to the README, the demo site, and the notes of all three existing releases (`demo-models-v1`–`v3` descend from the legacy LM, itself pretrained on a FineWeb-Edu slice). Every future model card carries it.
...
**PT-0 — DONE (throughput)** · `reports/pt/throughput.md`. Configs `configs/pt/pt_{35m,70m,150m}.yaml` (40.2 / 75.5 / 157.6M parameters, 32k vocab). The WDDM driver spills past 12 GB instead of raising OOM, so operating points stay ≤ 9 GB. `torch.compile` works via community `triton-windows` 3.5.1: compiled throughput 68.3k / 36.2k / 19.8k tok/s (35M b24 / 70M b16 / 150M b12), +32–52% over eager with lower memory. Projections: PT-2 ≈ 21 h locally; PT-3 150M × 10B ≈ 5.8 d locally (within 7 d and the disk budget); 150M × 20B ≈ 11.7 d locally, or ≈ $50–150 rented, **which needs owner approval**. The 20B disk footprint also exceeds 150 GB unless the pipeline streams, which is another owner decision. Nothing purchased.

**PT-DECISION (owner, 26 September)** · PT-3 is **approved at 10B tokens locally**; no rented compute. The plateau-stop rule stays; if the downstream probe is still rising at 10B, report the slope and ask again before extending to 20B on rented GPUs. Build the streaming data pipeline now so disk is not a ceiling. Pending owner action: switching the NVIDIA "CUDA Sysmem Fallback Policy" to "Prefer No Sysmem Fallback"; after the owner confirms, re-test the VRAM ceiling (overflow should raise OOM) and re-tune batch sizes if safe.

**ENV-1 (compiled-vs-eager parity + pinned environment)** · `torch.compile` (triton-windows **3.5.1.post24**, PyTorch 2.14.0+cu132) against eager on random-init LMs, the same bf16 batch (`reports/pt/compile_parity.json`): pt_35m |Δloss| 2.8e-4, hidden relative diff 0.56% (max abs 0.028), gradient relative diff 0.89%, gradient cosine 0.99996; pt_150m 4.2e-4, 0.89%, 1.27%, 0.99992. **Conclusion: equivalent within bf16 rounding** (fused kernels reorder reductions); compiled training is approved for PT. Versions are pinned in `pyproject.toml` (extra `pt`) and `docs/ENVIRONMENT.md`.

**PT-STREAM (pipeline)** · `scripts/pt/stream_corpus.py`: download (pinned, SHA-256 verified) → dedup (persistent corpus-wide SHA-1 set seeded from PT-1's cleaned docs) → decontaminate (same code as PT-1) → tokenize (`pt_32k`, hash-checked) → uint16 shards in one pass. No cleaned text is persisted; `--delete-raw` removes each parquet after its tokens are committed; resumable via `stream_manifest.json`. Tested on a synthetic parquet (2 duplicates and 1 contaminated doc removed; shards written; rerun skips processed inputs). Disk now scales with shards only (2 bytes/token): 10B tokens ≈ 20 GB. Launched on `sample/10BT` files 002–013 with `--delete-raw` in the background (CPU), continuing shard numbering after `train_0016`.
