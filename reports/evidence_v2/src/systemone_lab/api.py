...
def systemone(req: Request):
    ensure_loaded(); packed=pack_request(tok,req.state,req.questions,device=device); mask=branch_attention_mask(packed.branch_ids)
    with torch.no_grad():
        h=model.hidden(packed.input_ids[None,:],packed.position_ids[None,:],mask)
        answers={}
        for layout in packed.layouts:
            logits=model.decision_logits(h,layout.decide_position,layout.option_end_positions); p=F.softmax(logits.float(),-1).cpu().tolist(); probs=dict(zip(layout.option_keys,p)); best=max(range(len(p)),key=p.__getitem__)
